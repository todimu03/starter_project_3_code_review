...
class HashTable:
    ...
    def hashFunction(self, key, probe):
        """Hash function that uses linear probing, key is a string"""
        value = 0
        for char in key:
            value += ord(char)
        return (value + probe) % self.size

    def add(self, key, value):
        """Adds a new key value pair to the hash table"""
        if(self.amount_filled / self.size > 0.7):
    ...
    def rehash(self, new_size):
        ...
        # remake the array at a new size, then add all previous key value pairs
        new_array = [None] * new_size
        temp = self.array
        self.array = new_array
        self.size = new_size
        self.amount_filled = 0   # reset count before readding table
        for i in temp:
            if i is not None:
                self.add(i[0], i[1])
...
class Boggle:
    ...
    def getSolution(self):
        ...
        or type(self.grid[0]) is not list
        or self.hashTable is None):
            return set()
        ...
        for i in range(len(self.grid)):
            if len(self.grid) != len(self.grid[i]):
                return set()
        ...
        for i in self.grid:
            for j in i:
                if ((j not in a_to_z) 
                and (j not in special_valid_cases)
                or  (j in special_invalid_cases)):
                    return solved_list
    ...
    def solve(self, word, last_place, used_tiles, solved_list):
        ...
        # continue if there are valid_words that word could still be
        ...
        for i in range(len(adjacent_letters)):
            ...
            # duplicate the used tiles without reference
            temp_used_tiles = used_tiles.copy()
    ...
    def check_substring(self, string, solved_list):
        """return true if valid prefix or actual word"""
